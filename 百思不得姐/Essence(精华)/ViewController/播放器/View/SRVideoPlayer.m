...
@property (nonatomic, assign) BOOL dc_isFullScreen;
...
- (UIButton *)replayBtn {
    ...
    if (!_replayBtn) {
        ...
        [_replayBtn setImage:[UIImage imageNamed:@"replay"] forState:UIControlStateNormal];
    ...
    if (pan.state == UIGestureRecognizerStateChanged) {
        ...
        if (_controlType == SRControlTypeProgress) {
            ...
            if (videoCurrentTime > _videoCurrent) {
                [self.videoProgressTip setTipImageViewImage:[UIImage imageNamed:@"progress_right"]];
            } else if(videoCurrentTime < _videoCurrent) {
                [self.videoProgressTip setTipImageViewImage:[UIImage imageNamed:@"progress_left"]];
            }
...
- (void)videoTopBarDidClickCloseBtn {
    ...
    [self destroyPlayer];
    if (_delegate && [_delegate respondsToSelector:@selector(selectedWithCloseBtn)]) {
        [_delegate selectedWithCloseBtn];
    }
}
